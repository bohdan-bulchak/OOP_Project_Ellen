...
public class KeeperController implements KeyboardListener {
    ...
    public void keyPressed(@NotNull Input.Key key) {
        ...
        if (key == Input.Key.U && keeper.getScene() != null) {
            Usable<?> usable = keeper.getScene().getActors().stream().filter(Usable.class::isInstance).
                filter(keeper::intersects).map(Usable.class::cast).findFirst().orElse(null);
            if (usable != null) {
